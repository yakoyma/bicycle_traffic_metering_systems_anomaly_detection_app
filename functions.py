...
def evaluate_binary_classification(y_test, y_pred, y_proba, labels):
    ...
    print('Lift Score: {}'.format(lift_score(
        y_target=y_test, y_predicted=y_pred)))
    ...
    if y_proba is not None:
        ...
        print('Gini: {:.3f}'.format(2 * roc_auc - 1))
...
class StopOptimisationEarlyCallback:
    ...
    improvement after a set number of trials.

    Attributes:
        stagnation_threshold (int): the number of trials without improvement
                                    before stopping
