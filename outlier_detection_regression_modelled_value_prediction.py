...
bicycle traffic metering systems in Nantes - Regression to predict the feature
Modelled value
===============================================================================

This file is organised as follows:
1. Load the training and test sets (both features and targets)
2. Hyperopt-sklearn
3. Perpetual ML
"""
# Standard libraries
...
if __name__ == '__main__':
    ...
    TRAIN_INPUT_CSV = 'datasets/outlier detection/train_dataset.csv'
    TEST_INPUT_CSV = 'datasets/outlier detection/train_dataset.csv'
    ...
    # Model persistence
    dump(model, 'models/outlier detection/regression/hpsklearn/model.joblib')
    ...
    # Train score
    print('\nTrain score: {}'.format(model.base_score))

    # Make predictions
    y_pred = model.predict(X_test)

    # Best hyperparameters of the model
    print('\nBest hyperparameters: {}'.format(model.get_params()))
    ...
    print('Gain: {}'.format(model.calculate_feature_importance('Gain')))

    # Model persistence
    dump(
        model,
        'models/outlier detection/regression/perpetualbooster/model.joblib'
    )
